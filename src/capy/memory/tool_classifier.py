...
from capy.prompts.tool_call_system_prompt import TOOL_CALL_SYSTEM_PROMPT
...
class ToolDecision:
    ...
    Attributes:
        action: One of "ADD", "UPDATE", "REPLACE", "DELETE", "NOOP"
        memory_id: ID of memory to update/delete (None for ADD/NOOP)
        text: Text to store for ADD/UPDATE/REPLACE (None for DELETE/NOOP)
    """
    action: str
