...
from capy.memory.add.add_extraction_phase import extraction_phase
from capy.memory.add.add_updation_phase import update_phase
...
class CapyMemory:
    def __init__(self, db: Session):
        """
        Initialize CapyMemory with a database session.
    ...
    def search(self, query: str, conversation_id: int, limit: int = 10, include_connections: bool = True) -> Dict:
        ...
        if include_connections:
            for _, mem in top_results:
                connections = (mem.memory_metadata or {}).get("connections", {})
                if not isinstance(connections, dict):
                    continue

                conn_ids = connections.get("bubble_ids", [])
        ...
        for score, mem in top_results:
            ...
            results.append({
                ...
                "score": round(score, 4),
                "connections": connections.get("bubble_ids", []),
            })
