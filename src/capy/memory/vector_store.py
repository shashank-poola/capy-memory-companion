...
class FAISSVectorStore:
    ...
    Attributes:
        dimension: The size of the configured embedding vectors
    ...
    def __init__(self, dimension: int = 1536):
        ...
        Args:
            dimension: Size of the configured embedding vectors
        """
...
def get_embedding_dimension() -> int:
    """Return the default vector dimension for the configured provider."""
    settings = get_settings()
    return 384 if settings.embedding_provider == "local" else 1536


def get_index_path(conversation_id: int) -> str:
    """Get the file path for a conversation's index."""
    index_dir = os.path.join(os.getcwd(), "data", "capy_memory", "indexes")
...
def get_vector_store(conversation_id: int) -> FAISSVectorStore:
    ...
    This is the main entry point for using FAISS in Capy.
    ...
    if conversation_id not in _vector_stores:
        store = FAISSVectorStore(dimension=get_embedding_dimension())
